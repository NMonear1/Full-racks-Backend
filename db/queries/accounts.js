...
export async function updateAccount({ id, user_id, type, account_number, balance, created_at }) {
    const sql = `
    UPDATE accounts SET
        user_id = $2,
        type = $3,
    ...
    RETURNING *;
    `;
    const { rows: [account] } = await db.query(sql, [id, user_id, type, account_number, balance, created_at]);
    return account;
}
